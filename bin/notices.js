...
function request_list() {
	DNJS.ajax.parse(`admin/ajax.php?list=${type}&page=${page}`, function(arr){
		if(arr.length == 0 && page == 0) { id('load').innerHTML = 'No Notices'; }
...
function build_item(e) {
	...
	html += `<div class="ntcprg">${getPara(e.body)}<br>`;
...
window.addEventListener('load', function(){
	...
	if(opn) { view_notice(opn); return; }
		...
		if(type.indexOf('opa') > -1) {
			id('title').innerHTML = 'DCOPA Notices';
		} else {
			id('title').innerHTML = 'All Notices';
		}
		request_list();
	}
});
...
function view(nid) {
	if(type.indexOf('.') > -1) {
		location = 'notices.opa.php?id=' + nid;
	} else {
		location = 'notices.php?id=' + nid;
	}
}
